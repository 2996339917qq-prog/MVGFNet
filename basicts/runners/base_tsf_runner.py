...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def log_losses(self, epoch, main_loss, smooth_loss):
        save_path = "loss_records.csv"
        write_header = not os.path.exists(save_path)

        with open(save_path, mode="a", newline="") as f:
            writer = csv.writer(f)
            if write_header:
                writer.writerow(["epoch", "main_loss", "smooth_loss"])
            writer.writerow([epoch, main_loss, smooth_loss])
    ...
    #     iter_num = (epoch - 1) * self.iter_per_epoch + iter_index
    #     missing_rate = 0.03
    #     mask = torch.rand_like(data) > missing_rate  # True 保留，False 缺失
    #     data = data * mask
    #     prediction, real_value, smooth_loss, _ = self.forward(
    #         data=data, epoch=epoch, iter_num=iter_num,
    #         train=True, return_patch_error=False
    #     )
    #
    #     # 反归一化
    ...
    #     loss = main_loss + 0.05 * smooth_loss
    #
    #     # # 记录主 metrics
    #     # for metric_name, metric_func in self.metrics.items():
    ...
    #     #     self.update_epoch_meter("train_" + metric_name, metric_item.item())
    #     #
    #     # # 每个epoch的第一个batch保存一次损失
    #     # if iter_index == 0 and epoch not in self._logged_epochs:
    #     #     self.log_losses(epoch, main_loss.item(), smooth_loss.item())
    ...
    def train_iters(self, epoch: int, iter_index: int, data):
        ...
        missing_rate = 0.03
        mask = torch.rand_like(history_data) > missing_rate
        history_data = history_data  * mask
        data =(future_data,history_data)
        # -------- 2. 前向传播 --------
        prediction, real_value= self.forward(
            data=data,  # 注意：使用带缺失的数据训练
            epoch=epoch,
            iter_num=iter_num,
            train=True,
            return_patch_error=False
        )
        ...
        # -------- 4. Curriculum Learning 机制（可选） --------
        if self.cl_param:
            cl_length = self.curriculum_learning(epoch=epoch)
            prediction_rescaled = prediction_rescaled[:, :cl_length, :, :]
            real_value_rescaled = real_value_rescaled[:, :cl_length, :, :]

        # -------- 5. 主损失 + Smooth loss 加权 --------
    ...
    def test(self):
        ...
        prediction = []
        real_value = []
        for n, data in enumerate(self.test_data_loader):
...
#         num_samples=864,
#         start_idx=16128,
#         base_time=datetime(2016, 8, 26, 0, 0),
#         save_path=None,
# ):
#     if prediction.ndim == 4:
#         prediction = prediction.squeeze(-1)
#     if real_value.ndim == 4:
#         real_value = real_value.squeeze(-1)
#
#     indices = [start_idx + i * stride for i in range(num_samples)]
#     indices = [i for i in indices if i < prediction.shape[0]]
#
#     pred_seq = prediction[indices, :, sensor_id].reshape(-1).cpu().numpy()
#     true_seq = real_value[indices, :, sensor_id].reshape(-1).cpu().numpy()
#
#     time_index = [base_time + timedelta(minutes=5 * i) for i in range(len(true_seq))]
#
...
#
#     # 绘制曲线
#     plt.plot(time_index, true_seq, label='Ground Truth', linewidth=2.5, color="#0055A4")
#     plt.plot(time_index, pred_seq, label='Prediction', linewidth=2.0, color="#E41A1C", alpha=0.9)
#
#     # 填充误差区域
#     plt.fill_between(time_index, pred_seq, true_seq, color="#B0C4DE", alpha=0.3)
#
#     plt.ylabel('Traffic Flow', fontsize=14)
#     ax = plt.gca()
#     ax.set_xlim(base_time, base_time + timedelta(days=2))
#     ax.set_xticks([base_time + timedelta(hours=6 * i) for i in range(9)])
...
#
#     # 图例放图内偏中间，不遮挡曲线
#     plt.legend(fontsize=12, loc='upper left', bbox_to_anchor=(0.5, 0.95), frameon=False)
#
#     plt.tight_layout()
#
